...
export default function ContactForm() {
  ...
  const handleWhatsApp = () => {
    if (!isValid) return
    const text = [
      '📋 *Nova solicitação de crédito - PrimeCred*',
      '',
      `👤 *Nome:* ${form.name}`,
      `📞 *Telefone:* ${form.phone}`,
      form.email ? `📧 *E-mail:* ${form.email}` : null,
      `💳 *Tipo de crédito:* ${form.loanType}`,
      `💰 *Valor desejado:* ${form.amount}`,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div>
            ...
            <p className="text-slate-400 leading-relaxed mb-8">
              ...
              com a melhor proposta para você. Consulta gratuita e sem compromisso.
            </p>
            ...
            <div className="flex flex-col gap-4">
              ...
              <ContactInfo
                icon="📸"
                label="Instagram"
                value="@thaironyprimecred"
                href="https://instagram.com/thaironyprimecred"
              />
            </div>

            <div className="mt-8 bg-slate-800 border border-slate-700 rounded-2xl p-5">
              <p className="text-sm text-slate-400 leading-relaxed">
                <span className="text-amber-400 font-semibold">🔒 Seus dados estão seguros.</span>{' '}
                Utilizamos criptografia de ponta a ponta. Suas informações nunca
                são compartilhadas com terceiros sem a sua autorização.
              </p>
            </div>
          </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 md:p-8">
            {status === 'success' ? (
              ...
              <form onSubmit={handleEmail} className="flex flex-col gap-5">
                ...
                {/* Row 3 */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Tipo de crédito *" id="loanType">
                    <select
                      id="loanType"
                      value={form.loanType}
                  ...
                  <Field label="Valor desejado *" id="amount">
                    <select
                      id="amount"
                      value={form.amount}
                      onChange={set('amount')}
                ...
                {status === 'error' && (
                  <p className="text-red-400 text-sm bg-red-400/10 border border-red-400/20 rounded-lg px-4 py-3">
                    Ocorreu um erro ao enviar. Tente pelo WhatsApp abaixo.
                  </p>
                )}
...
const inputClass =
  'w-full bg-slate-900 border border-slate-600 hover:border-slate-500 focus:border-amber-400 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm outline-none transition-colors'
