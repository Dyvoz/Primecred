      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            ...
            <p className="mt-4 text-slate-400 text-sm leading-relaxed">{T.footer.description}</p>
            <div className="flex gap-4 mt-6">
              <a href="https://instagram.com/thaironyprimecred" target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                className="w-10 h-10 rounded-full bg-slate-800 hover:bg-gradient-to-br hover:from-purple-600 hover:to-pink-500 flex items-center justify-center text-slate-400 hover:text-white transition-all">
                <InstagramIcon />
              </a>
              <a href={`https://wa.me/${WA_NUMBER}`} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"
                className="w-10 h-10 rounded-full bg-slate-800 hover:bg-green-600 flex items-center justify-center text-slate-400 hover:text-white transition-all">
                <WhatsAppIcon />
              </a>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2">
              {T.footer.navLinks.map((label, i) => (
                <li key={label}>
                  <a href={T.footer.navHrefs[i]} className="text-slate-400 hover:text-amber-400 text-sm transition-colors">{label}</a>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="flex flex-col gap-3">
              <li>
                <a href={`https://wa.me/${WA_NUMBER}`} target="_blank" rel="noopener noreferrer"
                  ...
                  <WhatsAppIcon />+55 22 98844-9328
                </a>
              </li>
              <li>
                <a href="https://instagram.com/thaironyprimecred" target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 text-slate-400 hover:text-pink-400 text-sm transition-colors">
                  <InstagramIcon />@thaironyprimecred
                </a>
              </li>
            </ul>
            <a href="#contato"
        ...
        <div className="mt-10 pt-8 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-slate-500 text-xs">© {year} PrimeCred. {T.footer.rights}</p>
          <p className="text-slate-600 text-xs">{T.footer.disclaimer}</p>
        </div>
      </div>
    </footer>
  )
